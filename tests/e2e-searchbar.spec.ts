...
test.describe("SearchBar filters", () => {
  ...
  test("result page preserves selected filters and pagination query", async ({ page }) => {
    ...
    await page.goto(`${baseURL}/homestays?guests=3&type=Ph%C3%B2ng&type=L%E1%BB%81u+Glamping&amenities=Wifi+mi%E1%BB%85n+ph%C3%AD&price=2000000`);
    ...
    const detail = page.getByRole("link", { name: /Chi tiết|Xem chi tiết/i }).first();
    if (await detail.count()) {
      await expect(detail).not.toHaveAttribute("href", /checkIn=/);
      await expect(detail).not.toHaveAttribute("href", /guests=/);
    }
