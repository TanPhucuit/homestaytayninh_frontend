...
export default async function OwnerProxyBookingPage({ searchParams }: { searchParams: Promise<FlashSearchParams> }) {
  ...
  if (!canAccess(user.role, [...allowed])) {
    return <AccessDenied description="Đặt hộ khách hàng chỉ dành cho Owner, Owner Staff hoặc Admin." />;
  }

  const homestays = await getOwnerHomestays(user.role);

  return (
    <OwnerShell title="Đặt hộ khách hàng" description="Owner hoặc nhân viên homestay tạo booking và chọn dịch vụ hộ khách gọi điện hoặc đặt trực tiếp tại quầy." flash={flash}>
