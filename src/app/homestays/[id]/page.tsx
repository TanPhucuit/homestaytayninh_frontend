    ...
    <main className="min-h-screen text-[#1c1c19]">
      ...
      <section className="mx-auto max-w-7xl px-4 py-8 md:px-8">
        <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          ...
          <Link className="btn-primary" href={`/checkout?homestayId=${homestay.id}`}>
            Đặt phòng
          </Link>
        </div>
        ...
        <div className="grid gap-4 lg:grid-cols-[1.35fr_0.65fr]">
          <div
            className="min-h-[520px] rounded-[28px] bg-cover bg-center shadow-[0_28px_80px_rgba(154,64,41,0.16)]"
      ...
      <section className="mx-auto grid max-w-7xl gap-8 px-4 pb-16 md:px-8 lg:grid-cols-[1fr_380px]">
              ...
              {homestay.rooms.map((room) => (
                <article className="rounded-[24px] bg-[#fdf9f4] p-5" key={room.id}>
                  ...
                  <div className="mt-5 flex items-end justify-between gap-4">
                    ...
                    <Link className="btn-secondary" href={`/checkout?homestayId=${homestay.id}&roomId=${room.id}`}>
                      Chọn phòng
                    </Link>
                  </div>
                </article>
        ...
        <aside className="h-fit rounded-[24px] bg-white p-6 shadow-[0_24px_80px_rgba(123,41,20,0.1)] lg:sticky lg:top-28">
          ...
          {mainRoom && (
            <Link className="btn-primary mt-6 w-full" href={`/checkout?homestayId=${homestay.id}&roomId=${mainRoom.id}`}>
              Tiếp tục đặt phòng
            </Link>
          )}
